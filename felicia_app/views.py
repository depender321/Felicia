...
def getPrice(request):
    ...
    if request.method == "GET" and request.is_ajax():
        price = Product.objects.exclude(price__isnull=True). \
            exclude(price__exact='').order_by('price').values_list('price').distinct()
...
def invoice(request,orderid):
    ...
    print(order1)
    return render(request,'invoice.html',{'order1':order1})

def checkoutform(request):
    return render(request,'checkoutform.html')

def kurti(request):
    k = Product.objects.all().filter(category="Kurti")
    context = {
        'kurti': k,
    }
...
def contact(request):
    if request.method=='POST':
        ...
        info1=contactus(name=Name,email=Email,subject=Subject,description=Description)
        info1.save()
